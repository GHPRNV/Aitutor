...
import { debugFlow, buildContext } from '@/lib/ai/flows';
import { SEED_PROBLEMS } from '@/lib/data/seed-problems';

export async function POST(req: NextRequest) {
  try {
    const { code, problemId, language, execution } = await req.json();
    ...
    const context = buildContext({
      ...
      code,
      language,
      execution: execution || 'No execution result available — code has not been run yet.',
    });

    const output = await debugFlow({ context });
